...
(() => {
  ...
  window.openDatePicker = function (button) {
    ...
    if (
      checkbox.checked &&
      isInGrayState(associatedButton) &&
      !modifiedButtons.has(associatedButton)
    ) {
      selectedButton = associatedButton;
      document.getElementById("date-modal").style.display = "flex";
    } else {
      alert(
        "Solo puedes modificar la fecha si el estado está en gris y el checkbox está marcado, y si no se ha modificado antes."
      );
    }
  };
  ...
  window.saveDates = function () {
    ...
    if (startDate && endDate && new Date(startDate) <= new Date(endDate)) {
      ...
      const buttonContent = selectedButton.textContent
        .split("Fecha de inicio")[0]
        .trim();
      ...
      alert(
        "Por favor, selecciona ambas fechas y asegúrate de que la fecha inicial no sea posterior a la fecha final."
      );
    }
  };
